...
let assets = { snapp: 0, hami: 0, farabi: 0, card: 0, wallet: 0 };
let logs = [];
let txs = [];
let history = [];
    ...
    if(raw){
      ...
      if(d && typeof d === 'object' && d.enc === true && d.ct && d.iv && d.salt){
        ...
        if(d.assets && typeof d.assets === 'object' && !Array.isArray(d.assets)){
          // جایگزینی کامل — مقادیر پیش‌فرض اولیه نباید روی دادهٔ ذخیره‌شده باقی بمانند
...
function applyStatePayload(d){
  ...
  if(d.assets && typeof d.assets === 'object' && !Array.isArray(d.assets)){
    // جایگزینی کامل — جلوگیری از نشت مقادیر پیش‌فرض اولیه روی دادهٔ واقعی
    assets = Object.assign({}, d.assets);
    Object.keys(assets).forEach(k => {
      const v = Number(assets[k]);
      assets[k] = (isFinite(v) && !isNaN(v)) ? v : 0;
